# weave — AI Assistant Instructions (Gemini)

This file is read by Gemini and other AI assistants when working in this repository. It describes the codebase, conventions, and how to work effectively in it.
...
- Adapter tests use fixture directories under `tests/fixtures/` — never write to the real `~/.gemini/` in tests
...
The adapters are opaque. They expose only the `CliAdapter` trait. The core does not know about `settings.json` or `GEMINI.md` — that's the adapter's concern.
...
- Do not write to `~/.gemini/`, `~/.claude/`, `~/.codex/`, or any real user config in tests
