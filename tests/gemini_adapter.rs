...
fn apply_omits_env_key_when_server_has_no_env_vars() {
    ...
    assert!(
        config["mcpServers"]["no-env-server"].is_object(),
        "no-env-server must have been written to mcpServers"
    );
    assert!(
...
fn apply_http_server_writes_url() {
    ...
    // Gemini does not use a "type" key for http servers
    assert!(
        entry["type"].is_null(),
        "type key must not appear in Gemini HTTP server config"
    );
}
